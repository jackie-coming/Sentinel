...
public class AuthValidator {
  ...
  /**
   * The specific rules are: (1) If there is a JWT rule matching the request, the request is
   * matched. If the matching result is rejected, the request is rejected. (2) If any DENY policy
   * matches the request, the request is denied. (3) ALLOW the request if the workload does not have
   * an Allow policy. (4) ALLOW any Allow policy if it matches the request.
   *
   * @param request
   * @param rules
   * @return
  ...
  public static boolean validate(UnifiedHttpRequest request, Rules rules) {
    ...
    for (JwtRule jwtRule : rules.getJwtRules().values()) {
      String token = JwtUtil.getTokenFromJwtRule(request.getParams(), request.getHeaders(),
          jwtRule);
