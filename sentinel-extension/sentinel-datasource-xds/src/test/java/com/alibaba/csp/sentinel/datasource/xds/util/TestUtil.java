...
public class TestUtil {
   ...
   * /TestUtil.java_
   * <p>
   * 2.Change the token in file
   * _sentinel-extension/sentinel-datasource-xds/src/test/resources/token_, and token path in
   * istio-proxy > cat /var/run/secrets/tokens/istio-token
   * <p>
   * 3.Change the root-cert.pem in file
   * _sentinel-extension/sentinel-datasource-xds/src/test/resources/root-cert.pem_，and root-cert.pem
   * path in istio-proxy > cat /var/run/secrets/istio/root-cert.pem
   * <p>
  ...
  public static XdsConfigProperties createConfig() throws URISyntaxException, IOException {
    ...
    xdsConfigProperties.setCaCertPath(
        TestUtil.class.getClassLoader().getResource("root-cert.pem").toString());
