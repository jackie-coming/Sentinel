...
public class XdsConfigProperties {
  ...
  /**
   * jwt token for istiod. If null, it will be read from the file according to jwtPolicy
   */
  ...
  /**
   * Null will be returned if the time elapsed during obtaining the certificate time unit second
   */
  private int getCertTimeoutS;

  /**
   * validity of certificate, time unit second
   */
  ...
  /**
   * the reconnection delay of the Xds client after disconnection. time unit second
   */
  private int reconnectionDelayS;

  /**
   * xds Client initialization await time time unit second
   */
  ...
  /**
   * Read the build configuration from the environment variable or use the default value if not
   * available
   */
  public static XdsConfigProperties getFromXdsPropertiesEnv() {
    ...
    Optional.ofNullable(System.getenv(ConfigConstant.ENV_ISTIOD_TOKEN))
        .ifPresent(c -> config.setIstiodToken(c));
    Optional.ofNullable(System.getenv(ConfigConstant.ENV_CA_ADDR))
        .ifPresent(c -> config.setCaAddr(c));
    Optional.ofNullable(System.getenv(ConfigConstant.ENV_CA_CERT_PATH))
        .ifPresent(c -> config.setCaCertPath(c));
    Optional.ofNullable(System.getenv(ConfigConstant.ENV_NAMESPACE))
        .ifPresent(c -> config.setNamespace(c));
    Optional.ofNullable(System.getenv(ConfigConstant.ENV_POD_NAME))
        .ifPresent(c -> config.setPodName(c));
    Optional.ofNullable(System.getenv(ConfigConstant.ENV_CLUSTER_ID))
        .ifPresent(c -> config.setClusterId(c));
