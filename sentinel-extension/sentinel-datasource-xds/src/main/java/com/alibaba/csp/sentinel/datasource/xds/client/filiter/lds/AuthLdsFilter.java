...
public class AuthLdsFilter extends AbstractLdsFilter {
    ...
    for (Listener listener : listeners) {
      ...
      for (FilterChain filterChain : listener.getFilterChainsList()) {
        for (Filter filter : filterChain.getFiltersList()) {
          ...
          HttpConnectionManager httpConnectionManager = unpackHttpConnectionManager(
    ...
    for (Principal andId : andIds.getIdsList()) {
      ...
      for (Principal orId : orIds.getIdsList()) {
        ...
        if (Principal.IdentifierCase.AUTHENTICATED == identifierCase) {
          if (null == orId.getAuthenticated() || null == orId.getAuthenticated()
              .getPrincipalName()) {
            continue;
          }
          ...
          if (stringMatcher != null) {
            orChildren.addChildren(
                new AuthRule(new AuthCondition(AuthType.AUTHENTICATED, stringMatcher)));
          }
          continue;
        }
        ...
        if (Principal.IdentifierCase.HEADER == identifierCase) {
          ...
          orChildren.addChildren(
              new AuthRule(new AuthCondition(AuthType.HEADER, headerName, stringMatcher)));
          continue;
        }
        ...
        if (Principal.IdentifierCase.REMOTE_IP == identifierCase) {
          ...
          orChildren.addChildren(new AuthRule(
              new AuthCondition(AuthType.REMOTE_IP,
                  MatcherUtil.convertIpMatcher(orId.getRemoteIp()))));
          continue;
        }
      ...
      for (Permission orRule : orRules.getRulesList()) {
        ...
        if (Permission.RuleCase.HEADER == rulecase) {
          ...
          if (LDS_HEADER_NAME_AUTHORITY.equals(headerName)) {
            ...
            if (stringMatcher != null) {
              orChildren.addChildren(
                  new AuthRule(new AuthCondition(AuthType.HOSTS, stringMatcher)));
            }
            continue;
          }
          ...
          if (LDS_HEADER_NAME_METHOD.equals(headerName)) {
            ...
            if (stringMatcher != null) {
              orChildren.addChildren(
                  new AuthRule(new AuthCondition(AuthType.METHODS, stringMatcher)));
            }
            continue;
          }
          RecordLog.warn("[XdsDataSource] Header parsing is not supported, headerName={}",
              headerName);
          continue;
        }
      }
  ...
  public boolean resolve(List<Listener> listeners) {
    ...
    RecordLog.info(
        "[XdsDataSource] Auth rules resolve finish, RBAC rules size: {}, Jwt rules size: {}",
