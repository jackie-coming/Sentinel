...
public class XdsChannel implements AutoCloseable {
  ...
  public XdsChannel(XdsConfigProperties xdsConfigProperties,
    ...
    try {
      ...
      this.channel = createManagedChannel(abstractCertManager.getCertPair(),
          xdsConfigProperties.getHost(),
  ...
  public static Node createNode(String podName, String namespace, String clusterId) {
    ...
    try {
      ...
      RecordLog.info(
          "[XdsDataSource] Connect istio success,and podName={},namespace={},clusterId={}", podName,
          namespace, clusterId);
    } catch (Exception e) {
  ...
  /**
   * Id according to envoy standard This id can be obtained by command > istioctl pc bootstrap
   * ${podName} -n ${namespace} -o json
   * <p>
   * There is usually a selector when the configuration is delivered for example: ''' spec:
   * <br> selector:
   * <br> <br> matchLabels:
   * <br> <br> <br> app: details
   * ''' Istio will select the appropriate pod push xds based on the selector. PodName in Id
  ...
  public static String createNodeId(String podName, String namespace) {
    ...
    return String.format("sidecar~%s~%s.%s~%s" + SVC_CLUSTER_LOCAL, ip, podName, namespace,
        namespace);
  }

  private static ManagedChannel createManagedChannel(CertPair certPair, String host, int port)
      throws SSLException {
    if (null == certPair || null == certPair.getRawCertificateChain()
        || null == certPair.getRawPrivateKey()) {
