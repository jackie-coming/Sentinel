...
public class OpenSergoDataSourceGroup {
  ...
  private void initializeDataSourceMap() {
    dataSourceMap.put(OpenSergoSentinelConstants.KIND_FLOW_RULE,
  ...
  public void subscribeTrafficRouterConfig(String namespace, String appName) {
    ...
    openSergoClient.subscribeConfig(key, new OpenSergoConfigSubscriber() {
      ...
      public boolean onConfigUpdate(SubscribeKey subscribeKey, Object dataList) {
        try {
          ...
          RecordLog.error("Subscribing OpenSergo config subscribeKey: {} enhance error: {}",
              subscribeKey, e);
        }
    ...
    RecordLog.info("Subscribing OpenSergo base traffic-router rules for target <{}, {}>", namespace,
        app);
  }
  ...
  private void prepareForBaseSubscription() {
    ...
    RecordLog.info("Subscribing OpenSergo base fault-tolerance rules for target <{}, {}>",
        namespace, app);
  }
  ...
  public synchronized SentinelProperty<List<FlowRule>> subscribeFlowRules() {
    ...
    SentinelProperty<List<FlowRule>> property = dataSourceMap.get(
        OpenSergoSentinelConstants.KIND_FLOW_RULE);
    if (subscribed) {
      return property;
    }

    OpenSergoSentinelFlowRuleSubscriber subscriber = new OpenSergoSentinelFlowRuleSubscriber(
        ruleAggregator);
    ...
    SubscribeKey concurrencyLimitKey = new SubscribeKey(namespace, app,
        ConfigKind.CONCURRENCY_LIMIT_STRATEGY);
  ...
  public synchronized SentinelProperty<List<DegradeRule>> subscribeDegradeRules() {
    ...
    SubscribeKey subscribeKey = new SubscribeKey(namespace, app,
        ConfigKind.CIRCUIT_BREAKER_STRATEGY);
  ...
  public synchronized void unsubscribeFlowRules() {
    ...
    SubscribeKey concurrencyLimitKey = subscribeKeyMap.remove(
        ConfigKind.CONCURRENCY_LIMIT_STRATEGY);
    ...
    public boolean onConfigUpdate(SubscribeKey subscribeKey, Object data) {
      ...
      switch (subscribeKey.getKind()) {
        ...
        case CONCURRENCY_LIMIT_STRATEGY:
          return ruleAggregator.updateConcurrencyLimitStrategy(
              (List<ConcurrencyLimitStrategy>) data);
        default:
          // Type mismatch
          return false;
      }
  ...
  static class OpenSergoSentinelCircuitBreakerRuleSubscriber implements OpenSergoConfigSubscriber {
    ...
    @Override
    public boolean onConfigUpdate(SubscribeKey subscribeKey, Object data) {
      if (subscribeKey.getKind() != ConfigKind.CIRCUIT_BREAKER_STRATEGY
