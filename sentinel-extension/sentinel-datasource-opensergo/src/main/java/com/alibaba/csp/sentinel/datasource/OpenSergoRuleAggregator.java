...
public class OpenSergoRuleAggregator {
  ...
  public synchronized boolean updateCircuitBreakerStrategy(
  ...
  public synchronized boolean updateConcurrencyLimitStrategy(
      List<ConcurrencyLimitStrategy> strategies) {
  ...
  private boolean handleFlowRuleUpdate() {
    ...
    List<FlowRule> rulesFromRateLimitStrategies = assembleFlowRulesFromRateLimitStrategies(
        ftRuleMapByStrategyKind.get(ConfigKind.RATE_LIMIT_STRATEGY.getSimpleKindName()),
        rateLimitStrategyMap);
    List<FlowRule> rulesFromThrottlingStrategies = assembleFlowRulesFromThrottlingStrategies(
        ftRuleMapByStrategyKind.get(ConfigKind.THROTTLING_STRATEGY.getSimpleKindName()),
        throttlingStrategyMap);
    List<FlowRule> rulesConcurrencyLimitStrategies = assembleFlowRulesFromConcurrencyLimitStrategies(
        ftRuleMapByStrategyKind
            .get(ConfigKind.CONCURRENCY_LIMIT_STRATEGY.getSimpleKindName()),
        concurrencyLimitStrategyMap);
  ...
  private boolean handleCircuitBreakerRuleUpdate() {
    ...
    // Update rules to upstream data-source.
    return dataSourceMap.get(OpenSergoSentinelConstants.KIND_CIRCUIT_BREAKER_RULE)
        .updateValue(rules);
  }
  ...
  private List<FlowRule> assembleFlowRulesFromConcurrencyLimitStrategies(
      List<FaultToleranceRule> ftRules,
    ...
    for (FaultToleranceRule ftRule : ftRules) {
      List<ConcurrencyLimitStrategy> strategies = ftRule.getStrategiesList().stream()
          .filter(
              e -> e.getKind().equals(ConfigKind.CONCURRENCY_LIMIT_STRATEGY.getSimpleKindName()))
  ...
  private FlowRule fillFlowRuleWithConcurrencyLimitStrategy(FlowRule rule,
  ...
  private DegradeRule fillDegradeRuleWithCbStrategy(DegradeRule rule,
